...
# Robots live under robots/<robot>/ (urdf, mjcf, meshes/visual), inside the
# lite_description ament package whose package.xml is at the repo root
# (franka_description layout). Generated by the converter from each robot's cad/.
...
def discover_parity_pairs() -> list[tuple[Path, Path]]:
    ...
    MJCF variants like a ``*_frames.xml`` that have no same-stem URDF are
